    ...
    while true do
		...
		for k,v in pairs(cfg.chestOrgs) do
			...
			if distance <= 2.5 then
				...
				DrawText3D(v.coords[1], v.coords[2], v.coords[3]+0.2, "PRESSIONE ~g~E~w~ PARA ABRIR",400)
			end
		end
    ...
    while true do
		...
		for k,v in pairs(cfg.storesLocs) do
			...
			if distance <= 1.5 then
				...
				DrawText3D(v.coords[1], v.coords[2], v.coords[3]+0.2, "PRESSIONE ~g~E~w~ PARA ACESSAR.",400)
			end
		end
...
Citizen.CreateThread(function()
    while true do
		...
		for k,v in pairs(droplist) do
			...
			if distance <= 15 then
				time = 5
				DrawMarker(28,v.x,v.y,cdz+0.1,0,0,0,180.0,0,0,0.05,0.05,0.05,236, 120, 3, 220,0,0,0,0)
