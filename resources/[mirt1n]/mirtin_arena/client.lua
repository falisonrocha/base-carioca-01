                ...
                if distance <= 15.0 then
                    ...
                    if distance <= 2.0 then
                        if IsControlJustPressed(0,38) and vida > 101 and not LocalPlayer.state.NovatMode then
                            local getWeapons = vRP.getWeapons()
                            if getWeapons and next(getWeapons) ~= nil then 
                                TriggerEvent('Notify','negado','Você não pode entrar em uma arena com uma arma equipada. Utilize /garmas e tente novamente.',5) 
                            else 
                                openedCoords = v
                                openNui()
                            end
...
-- FUNCTIONS
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
local function isInArenaArea()
    local coords = GetEntityCoords(PlayerPedId())

    for _, loc in pairs(config.locArenas) do
        if #(coords - loc) <= 60.0 then
            return true
        end
    end
...
exports("inArena", function(status)
    return in_arena > 0 
end)