...
local caminhoneiro = {
	["Caminhoneiro"] = {
		iniciar = vec3(-624.43,-1608.92,26.89),
		pegarcaminhao = vec3(-610.43,-1609.37,26.89),
	}
}
			...
			if not servico then
				...
				if distance2 <= 2.0 then
					...
					if IsControlJustReleased(1, 51) and segundos <= 0 then
						segundos = 10
						criarVehicle(-610.98,-1597.31,26.74,90.17,"trash3", false)
					end
				end
			end
		end
		...
		if servico then
			...
			for k,v in pairs(zonas) do
				...
				if distance <= 15.0 and v.visivel then
					time = 5
					DrawMarker(21,v.coords[1],v.coords[2],v.coords[3],0,0,0,0,180.0,130.0,1.0,1.0,0.5, 255,69,0,180 ,1,0,0,1)
