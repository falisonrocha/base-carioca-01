...
AddEventHandler("playerConnecting", function(name, setKickReason, deferrals)
	if not GeneratingCache then deferrals.update("[NATIVIDADE] Ops, houve um problema. tente novamente.") return end
	...
	if ids == nil or #ids == 0 then
		deferrals.done("[NATIVIDADE] Ocorreu um problema de identidade.")
		return
	end

	local user_id = vRP.getUserIdByIdentifiers(ids)
	if not user_id then
		deferrals.done("[NATIVIDADE] Ocorreu um problema de identificação.")
		return
	end
