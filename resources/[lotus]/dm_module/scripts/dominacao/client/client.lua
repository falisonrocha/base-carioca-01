...
function Ply:EnterZone(domination_id, running)
    ...
    if vTunnel.checkCooldown(domination_id) then
        Ply.inZone = true
...
RegisterTunnel.syncDeath = function(data)
    -- print(json.encode({ action = 'notifyKill', payload = { killer = data.killer, victim = data.victim, weapon = data.weapon }}) , { indent = true })
...
function showWall(p1, p2)
    ...
    DrawPoly(bottomLeft,topLeft,bottomRight,177,13,37,70)
    DrawPoly(topLeft,topRight,bottomRight,177,13,37,70)
    DrawPoly(bottomRight,topRight,topLeft,177,13,37,70)
    DrawPoly(bottomRight,topLeft,bottomLeft,177,13,37,70)
end
    ...
    while true do
        if Ply.inZone then 
            ...
            if IsPedInAnyVehicle(ped, false) then 
                ...
                for _, vName in ipairs(blockedVehicles) do 
                    if model == GetHashKey(vName) then 
                        print('deletar veiculo '..vName)
                        vTunnel._deleteVehicle(VehToNet(vehicle))
                    end
                end
            end
        end
