...
local zoomspeed = 3.5 -- camera zoom speed
...
local helicam = false
local polmav_hash = {
	[1] = GetHashKey("paramedicoheli"),
	[2] = GetHashKey("WRpolmav"),
}
			...
			if IsHeliHighEnough(heli) then
				...
				if IsControlJustPressed(0, toggle_rappel) then -- Initiate rappel
					...
					if GetPedInVehicleSeat(heli, 1) == lPed or GetPedInVehicleSeat(heli, 2) == lPed then
						...
						AddTextComponentString("~r~Não é possível fazer rapel nesse assento.")
...
function IsPlayerInPolmav()
	...
	for k in pairs(polmav_hash) do
		if IsVehicleModel(vehicle, polmav_hash[k]) then
			return IsVehicleModel(vehicle, polmav_hash[k])
		end
	end
